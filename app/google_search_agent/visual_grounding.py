...
async def annotate_image(query: str, tool_context: ToolContext) -> dict[str, str]:
    ...
    with _lock:
        ...
        if not image_bytes and _latest_images:
            fallback_sid = next(iter(_latest_images))
            image_bytes = _latest_images[fallback_sid]
            logger.info(
                "[Annotation] Session ID mismatch for camera frame — "
                "requested %s, using frame from %s",
                session_id,
                fallback_sid,
            )
    ...
    if not detections:
        return {
            "status": "success",
            "description": "I looked at the image but could not find what you asked about.",
        }
